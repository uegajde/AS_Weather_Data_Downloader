Partial Class Main
    ...
    Private Sub ReadConfig()
        ...
        Try
            ...
            TempString = LineInput(ConfigFileNum)  'Period(hour)=18
            Call GetPeriod()
            ''---D8-CWB_850hpa_WindSpeed_Streamline---
            D = 8
            TempString = LineInput(ConfigFileNum)  '---D8-CWB_850hpa_WindSpeed_Streamline---
            TempString = LineInput(ConfigFileNum)  'Enabled=true
            Call GetEnabled()
            TempString = LineInput(ConfigFileNum)  'ErrorLog=false
            Call GetErrorLog()
            TempString = LineInput(ConfigFileNum)  'Period(hour)=18
            Call GetPeriod()
            ''---D9-CWB_850hpa_RH_Streamline---
            D = 9
            TempString = LineInput(ConfigFileNum)  '---D9-CWB_850hpa_RH_Streamline---
            TempString = LineInput(ConfigFileNum)  'Enabled=true
            Call GetEnabled()
            TempString = LineInput(ConfigFileNum)  'ErrorLog=false
            Call GetErrorLog()
            TempString = LineInput(ConfigFileNum)  'Period(hour)=18
            Call GetPeriod()
            ''---D10-JMA_WaterVapor_Image---
            D = 10
            TempString = LineInput(ConfigFileNum)  '---D10-JMA_WaterVapor_Image---
            TempString = LineInput(ConfigFileNum)  'Enabled=true
            Call GetEnabled()
            TempString = LineInput(ConfigFileNum)  'ErrorLog=false
            Call GetErrorLog()
        ...
        Catch ex As Exception
            ...
            PrintLine(RecordNum, DateString + "_" + TimeString + "_" + Time + "_" + "Error occur when reading config")
            FileClose(RecordNum)
        End Try
    End Sub
